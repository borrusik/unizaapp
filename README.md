...
This project was born out of the need to unify the most important student systems into a single, clean, and modern interface. The application combines **Vzdelávanie** (schedule, grades, subjects), **WebKredit** (daily menu and ISIC card balance), and **UNIZA Student Mail**.
...
*   **✉️ Student Mail** — Integrated IMAP inbox and folders, search, safe message rendering, attachments, flags, and SMTP composition without opening Roundcube.
...
1.  **Encrypted sign-in:** Credentials are never stored in a database or in plaintext. When `UNIZA_SESSION_SECRET` is configured, the email and password are encrypted with authenticated AES-256-GCM in an `HttpOnly`, `Secure`, `SameSite=Strict` cookie. The cookie lasts only for the browser session unless the user enables automatic sign-in, which extends it to at most 30 days. Without this secret, IMAP/SMTP mail is disabled and only current upstream sessions are retained.
...
* **UNIZA Student Mail** through TLS-secured IMAP (`mail.stud.uniza.sk:993`) and authenticated SMTP (`mail.stud.uniza.sk:465`); credentials and message bodies remain server-side except for the messages explicitly shown to the signed-in user.
...
*   **React**
*   **TypeScript**
*   **Cheerio, ImapFlow, Nodemailer & MailParser** (for UNIZA integrations)
